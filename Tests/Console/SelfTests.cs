        ...
        try
        {
            ...
            foreach (var actor in new[] { admin, staff })
            {
                ...
                try
                {
                    ...
                    if (actor == admin)
                        await new AdminTicketController(auth, maintenance, employees).RunAsync(actor);
                    else
                        await new IncidentController(auth, maintenance, reports).RunAsync(actor);
                }
                finally
                {
